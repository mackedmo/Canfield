...
//Print the choices of actions if the player has won.
void Canfield::printWinMenu()
{
    cout << "Congratulations! You have won the game!" << endl;
    cout << "[q] quit [N] new game" << endl;
    cout << "Enter your choice: ";
}// Canfield::printWinMenu
...
void Canfield::performAction(char answer, int & score)
{
    switch (answer) 
    {
    case 'q':
...
void Canfield::printGame(int score)
{
    clearScreen();
    cout << "Deck:    ";
    if (waste != -1)
...
void Canfield::move(int & score)
{
    ...
    if (tableau[fromId].isEmpty())
        ...
        {
            card = reserve.pop();
            tableau[fromId].push(card);
        }
} // Canfield::move()

// Repeats last one move.
void Canfield::repeatMove(int & score)
{
    //checks that this is a move to repeat stored
    if (from)
    {
        int card;
        ...
        cout << "Error: Attempt to repeat nonexistent move." << endl;
} // Canfield::repeatMove()
...
int Canfield::getCard()
{
    ...
    case 't':
        if (!tableau[fromId].isEmpty())
            ...
            return -1;
    case 'T':
        if (!tableau[fromId].isEmpty() && (to == 'f' || to == 'F'))
            return tableau[fromId].top();
        else if (!tableau[fromId].isEmpty())
...
// Mutator method.
void Canfield::addCard(int card, int & score)
{
    if (from == 'T' && (to == 't' || to == 'T')) 
    {
    ...
    {   
        //adds cards to foundation and removes from tableau until complete
        foundation[toId].push(card);
        while (!tableau[fromId].isEmpty())
        {
            card = tableau[fromId].pop();
            foundation[toId].push(card);
            score++;
        }
    }    
    else if (from == 't' || from == 'd' || from == 'r') 
        if (to == 't' || to == 'T') 
